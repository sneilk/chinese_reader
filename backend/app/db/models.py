...
class Sentence(Base):
    ...
    # Задел, не реализовано: колонка есть, писать и читать её некому.
    #
    # Замысел (translation.md §5): глоссарий влияет на результат перевода, и по
    # слепку глоссария видно, какие предложения переведены под старую версию
    # словаря и просятся на перевод заново. Пока правка словаря к уже
    # переведённой главе действительно не применяется — но не «молча», как
    # обещал прежний комментарий, а просто никак.
...
class WordOccurrence(Base):
    """Сколько раз слово встретилось в главе. **Задел, не реализовано.**

    Замысел: агрегат вместо таблицы токенов (segmentation.md §6) — книга дала
    бы сотни тысяч строк, а вопросы к ним всего два: «где ещё встречалось это
    слово» и «сколько раз».

    Сейчас таблица заведена и мигрирована, но **никто в неё не пишет и никто
    её не читает**. Раньше здесь стояло «наполняется при разборе главы», и это
    было неправдой: следующий читатель исходил бы из того, что механизм есть.
    Пусть лучше пусто и честно, чем обещание, которого нет.
    """
...
class TranslationUsage(Base):
    ...
    занижен; потолок мягкий, и на один запрос это допустимо.

    «На один» — теперь буквально. Глава длиннее батча уезжает несколькими
    запросами, и раньше отказ на любом из них выбрасывал **все** предыдущие:
    подтверждённые, оплаченные и уже разобранные. Месячный счётчик их не
    видел, потолок на главу оставался нулевым, а повтор слал то же самое
    заново. Теперь успевшая часть доезжает до конвейера вместе с отказом
    (`TranslateFailure.partial`) и записывается здесь; незаписанным остаётся
    ровно один запрос — тот, чей ответ так и не пришёл.
    """
