...
describe('ответ не от сервиса', () => {
  // `http_502` заводит сам клиент, когда в ответе нет нашего {error:{kind}}:
  // отвечал прокси, а не бэкенд. На выкладке это происходит каждый раз.
  it.each(['http_404', 'http_500', 'http_502', 'http_503', 'http_504'])(
    '%s объясняется выкладкой, а не отправкой к разработчику',
    (kind) => {
      const info = describeError(kind)

      expect(info.title).not.toBe('Непонятная ошибка')
      expect(info.retryable).toBe(true)
      expect(info.advice).toContain('выкладка')
    },
  )

  it('настоящий неизвестный код остаётся непонятной ошибкой', () => {
    // Выдумывать объяснение тому, чего мы не знаем, хуже честного «покажите
    // разработчику»: совет обязан быть верным, а не успокаивающим.
    expect(describeError('какая-то новая беда').title).toBe('Непонятная ошибка')
  })

  it('свой kind по-прежнему сильнее: у него объяснение точнее', () => {
    expect(describeError('challenge').title).toBe('Сайт просит пройти проверку')
  })
})
